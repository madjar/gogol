...
-- open range \"\< range.max\" is desired, set range.min = -infinity. If
...
-- the MetricKind of the metric value. The end time must not be earlier
-- than the start time. When writing data points, the start time must not
-- be more than 25 hours in the past and the end time must not be more than
-- five minutes in the future. For GAUGE metrics, the startTime value is
-- technically optional; if no value is specified, the start time defaults
-- to the value of the end time, and the interval represents a single point
-- in time. If both start and end times are specified, they must be
-- identical. Such an interval is valid only for GAUGE metrics, which are
-- point-in-time measurements. The end time of a new interval must be at
-- least a millisecond after the end time of the previous interval. For
-- DELTA metrics, the start time and end time must specify a non-zero
...
-- must specify a a non-zero interval, with subsequent points specifying
