...
-- \`TransferOperation\` The \`PubsubMessage.data\` will contain a
...
-- | Required. The \`Topic.name\` of the Cloud Pub\/Sub topic to which to
...
-- guidelines: * If \`schedule_end_date\` and schedule_start_date are the
-- same and in the future relative to UTC, the transfer is executed only
-- one time. * If \`schedule_end_date\` is later than
-- \`schedule_start_date\` and \`schedule_end_date\` is in the future
-- relative to UTC, the job will run each day at start_time_of_day through
-- \`schedule_end_date\`.
...
-- starting jobs at or near midnight UTC it is possible that a job will
-- start later than expected. For example, if you send an outbound request
-- on June 1 one millisecond prior to midnight UTC and the Storage Transfer
-- Service server receives the request on June 2, then it will create a
...
-- only to Cloud Data Sources such as S3, Azure, and Cloud Storage. The
...
-- rejected with the error INVALID_ARGUMENT. Updating a job satus to
...
-- the field is not set, the job will never execute a transfer, unless you
...
-- name is used as the unique name for this job. If the specified name is
-- in use by a job, the creation request fails with an ALREADY_EXISTS
-- error. This name must start with \`\"transferJobs\/\"\` prefix and end
-- with a letter or a number, and should be no more than 128 characters.
-- Example: \`\"transferJobs\/[A-Za-z0-9-._~]*[A-Za-z0-9]$\"\` Invalid job
...
-- JobConfig. Present if and only if at least one TransferOperation has
...
-- such, it should generally not begin with a \'\/\'. (must meet Object
-- Name
...
awsS3Data =
  AwsS3Data'
    {_asdPath = Nothing, _asdBucketName = Nothing, _asdAwsAccessKey = Nothing}
...
-- | Required. Input only. AWS access key used to sign the API requests to
...
-- bytes, the object transfer will fail. * Ensure that each URL you specify
-- is publicly accessible. For example, in Cloud Storage you can [share an
-- object publicly]
