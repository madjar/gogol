...
-- TemplateVersion.
data ModifyTemplateVersionLabelRequestOp
...
instance Hashable ModifyTemplateVersionLabelRequestOp

instance FromHttpApiData ModifyTemplateVersionLabelRequestOp where
    parseQueryParam = \case
        "OPERATION_UNSPECIFIED" -> Right OperationUnspecified
        "ADD" -> Right Add
        "REMOVE" -> Right Remove
        x -> Left ("Unable to parse ModifyTemplateVersionLabelRequestOp from: " <> x)

instance ToHttpApiData ModifyTemplateVersionLabelRequestOp where
    toQueryParam = \case
        OperationUnspecified -> "OPERATION_UNSPECIFIED"
        Add -> "ADD"
        Remove -> "REMOVE"

instance FromJSON ModifyTemplateVersionLabelRequestOp where
    parseJSON = parseJSONText "ModifyTemplateVersionLabelRequestOp"

instance ToJSON ModifyTemplateVersionLabelRequestOp where
    toJSON = toJSONText
...
data JobCurrentState
    ...
    | JobStateResourceCleaningUp
      ...
      -- support team if you are intersted.
      deriving (Eq, Ord, Enum, Read, Show, Data, Typeable, Generic)
...
instance Hashable TemplateVersionType

instance FromHttpApiData TemplateVersionType where
    parseQueryParam = \case
        "TEMPLATE_TYPE_UNSPECIFIED" -> Right TemplateTypeUnspecified
        "LEGACY" -> Right Legacy
        "FLEX" -> Right Flex
        x -> Left ("Unable to parse TemplateVersionType from: " <> x)

instance ToHttpApiData TemplateVersionType where
    toQueryParam = \case
        TemplateTypeUnspecified -> "TEMPLATE_TYPE_UNSPECIFIED"
        Legacy -> "LEGACY"
        Flex -> "FLEX"

instance FromJSON TemplateVersionType where
    parseJSON = parseJSONText "TemplateVersionType"

instance ToJSON TemplateVersionType where
    toJSON = toJSONText
