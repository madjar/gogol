...
data ChromeOSDevice =
  ChromeOSDevice'
    ...
    , _codRecentUsers :: !(Maybe [RecentUsers])
...
codRecentUsers :: Lens' ChromeOSDevice [RecentUsers]
codRecentUsers
...
fiPrinterIds :: Lens' FailureInfo (Maybe Text)
fiPrinterIds
  = lens _fiPrinterIds (\ s a -> s{_fiPrinterIds = a})
...
-- | List of recent device users, in descending order, by last login time.
--
-- /See:/ 'recentUsers' smart constructor.
data RecentUsers =
  RecentUsers'
    { _ruEmail :: !(Maybe Text)
    , _ruType :: !(Maybe Text)
...
-- | Creates a value of 'RecentUsers' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'ruEmail'
...
recentUsers
    :: RecentUsers
recentUsers = RecentUsers' {_ruEmail = Nothing, _ruType = Nothing}


-- | The user\'s email address. This is only present if the user type is
-- \`USER_TYPE_MANAGED\`.
ruEmail :: Lens' RecentUsers (Maybe Text)
ruEmail = lens _ruEmail (\ s a -> s{_ruEmail = a})

-- | The type of the user.
ruType :: Lens' RecentUsers (Maybe Text)
ruType = lens _ruType (\ s a -> s{_ruType = a})

instance FromJSON RecentUsers where
        parseJSON
          = withObject "RecentUsers"
              (\ o ->
                 RecentUsers' <$> (o .:? "email") <*> (o .:? "type"))

instance ToJSON RecentUsers where
        toJSON RecentUsers'{..}
          = object
              (catMaybes
                 [("email" .=) <$> _ruEmail, ("type" .=) <$> _ruType])
