...
-- | Optional. A sync token, received from a previous \`ListDirectoryPeople\`
...
-- \`ListDirectoryPeople\` must match the call that provided the sync
-- token.
...
-- sync requests use the \`sync_token\` param instead.
...
-- | Optional. A page token, received from a previous \`ListDirectoryPeople\`
-- call. Provide this to retrieve the subsequent page. When paginating, all
-- other parameters provided to \`ListDirectoryPeople\` must match the call
