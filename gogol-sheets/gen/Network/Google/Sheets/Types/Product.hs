...
-- | The format of the text inside the bubbles. Strikethrough and underline
-- are not supported.
...
-- By default, applications SHOULD assume the sRGB color space. Note: when
-- color equality needs to be decided, implementations, unless documented
-- otherwise, will treat two colors to be equal if all their red, green,
-- blue and alpha values each differ by at most 1e-5. Example (Java):
