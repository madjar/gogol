...
data MattersHoldsListView
    ...
    | FullHold
      -- ^ @FULL_HOLD@
      -- Full representation of a Hold. Response includes all fields of \'BASIC\'
      -- and the entities the Hold applies to, such as accounts, or OU.
      deriving (Eq, Ord, Enum, Read, Show, Data, Typeable, Generic)
...
data MattersGetView
    ...
    | Full
      -- ^ @FULL@
      -- Full representation of matter is returned. Everything above and
...
data CountArtifactsRequestView
    ...
    | TotalCount
      -- ^ @TOTAL_COUNT@
      -- Response includes: total count, queried accounts count, matching
      -- accounts count, non-queryable accounts, queried account errors.
    | All
      -- ^ @ALL@
