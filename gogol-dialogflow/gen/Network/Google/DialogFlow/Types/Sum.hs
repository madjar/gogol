...
instance FromHttpApiData GoogleCloudDialogflowCxV3beta1TestCaseResultTestResult where
    parseQueryParam = \case
        "TEST_RESULT_UNSPECIFIED" -> Right TestResultUnspecified
        "PASSED" -> Right Passed
        "FAILED" -> Right Failed
...
instance FromHttpApiData GoogleCloudDialogflowCxV3RestoreAgentRequestRestoreOption where
    parseQueryParam = \case
        "RESTORE_OPTION_UNSPECIFIED" -> Right RestoreOptionUnspecified
        "KEEP" -> Right Keep
...
-- | Defines on what data we apply redaction. Note that we don\'t redact data
-- to which we don\'t have access, e.g., Stackdriver logs.
