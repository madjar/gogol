...
-- are listed below (case-sensitive): * all * App Engine * BigQuery * Cloud
-- Bigtable * Cloud Key Management Service * Compute Engine * Cloud
-- Dataflow * Cloud Identity and Access Management * Cloud Pub\/Sub * Cloud
-- Storage * Persistent Disk Note: These values are supported as input for
...
-- (\'all\', \'App Engine\', etc.). Note: \'all\' will enroll the resource
-- in all products supported at both \'GA\' and \'Preview\' levels. More
