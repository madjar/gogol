...
-- for better performance. App Engine co-location was only applicable to
...
-- | Encryption status specific to a backup. Applies only to Second
-- Generation instances.
...
-- means the deny maintenance period recurs every year. The date is in
...
-- located, either in a specific Compute Engine zone, or co-located with an
-- App Engine application. Note that if the preferred location is not
-- available, the instance will be located as close as possible within the
...
-- to bypass GTID consistency check between on-premises primary instance
