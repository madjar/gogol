...
-- | This feature is not generally available yet. View that specify that
-- partial response should be returned.
elView :: Lens' EnterprisesList (Maybe EnterprisesListView)
elView = lens _elView (\ s a -> s{_elView = a})

-- | This feature is not generally available yet. A token identifying a page
-- of results returned by the server.
elPageToken :: Lens' EnterprisesList (Maybe Text)
elPageToken
  = lens _elPageToken (\ s a -> s{_elPageToken = a})

-- | Required. This feature is not generally available yet. The ID of the
-- Cloud project of the EMM the enterprises belongs to.
elProjectId :: Lens' EnterprisesList (Maybe Text)
elProjectId
  = lens _elProjectId (\ s a -> s{_elProjectId = a})

-- | This feature is not generally available yet. The requested page size.
