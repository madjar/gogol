...
-- can be updated: *Mask* *Purpose* config.softwareConfig.pypiPackages
-- Replace all custom custom PyPI packages. If a replacement package map is
-- not included in \`environment\`, all custom PyPI packages are cleared.
-- It is an error to provide both this mask and a mask specifying an
-- individual package. config.softwareConfig.pypiPackages.packagename
-- Update the custom PyPI package packagename, preserving other packages.
-- To delete the package, include it in \`updateMask\`, and omit the
-- mapping for it in \`environment.config.softwareConfig.pypiPackages\`. It
...
-- a replacement environment variable map is not included in
