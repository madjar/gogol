...
data ProductPolicyAutoUpdateMode
    ...
    | AutoUpdateDefault
      ...
      -- a Wi-Fi network. * The device is charging * If the system update policy
      -- is set to \`WINDOWED\`: the local time of the device is within the daily
      -- maintenance window The device is notified about a new update within 24
      -- hours after it is published by the developer, after which the app is
