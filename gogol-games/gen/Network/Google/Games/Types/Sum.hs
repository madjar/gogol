...
data ResolveSnapshotHeadRequestResolutionPolicy
    ...
    | NoAutomaticResolution
      ...
      -- snapshot with manual resolution policy.
      deriving (Eq, Ord, Enum, Read, Show, Data, Typeable, Generic)

instance Hashable ResolveSnapshotHeadRequestResolutionPolicy

instance FromHttpApiData ResolveSnapshotHeadRequestResolutionPolicy where
    parseQueryParam = \case
        "RESOLUTION_POLICY_UNSPECIFIED" -> Right ResolutionPolicyUnspecified
        "USE_HEAD" -> Right UseHead
        "LONGEST_PLAYTIME" -> Right LongestPlaytime
        "MOST_RECENTLY_MODIFIED" -> Right MostRecentlyModified
        "HIGHEST_PROGRESS" -> Right HighestProgress
        "NO_AUTOMATIC_RESOLUTION" -> Right NoAutomaticResolution
        x -> Left ("Unable to parse ResolveSnapshotHeadRequestResolutionPolicy from: " <> x)

instance ToHttpApiData ResolveSnapshotHeadRequestResolutionPolicy where
    toQueryParam = \case
        ResolutionPolicyUnspecified -> "RESOLUTION_POLICY_UNSPECIFIED"
        UseHead -> "USE_HEAD"
        LongestPlaytime -> "LONGEST_PLAYTIME"
        MostRecentlyModified -> "MOST_RECENTLY_MODIFIED"
        HighestProgress -> "HIGHEST_PROGRESS"
        NoAutomaticResolution -> "NO_AUTOMATIC_RESOLUTION"

instance FromJSON ResolveSnapshotHeadRequestResolutionPolicy where
    parseJSON = parseJSONText "ResolveSnapshotHeadRequestResolutionPolicy"

instance ToJSON ResolveSnapshotHeadRequestResolutionPolicy where
    toJSON = toJSONText
...
instance FromHttpApiData ApplicationsGetEndPointEndPointType where
    parseQueryParam = \case
        "END_POINT_TYPE_UNSPECIFIED" -> Right EndPointTypeUnspecified
        "PROFILE_CREATION" -> Right ProFileCreation
        "PROFILE_SETTINGS" -> Right ProFileSettings
