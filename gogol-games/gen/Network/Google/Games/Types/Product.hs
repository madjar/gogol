...
-- game.
--
-- /See:/ 'snapshotCoverImageResource' smart constructor.
data SnapshotCoverImageResource =
  SnapshotCoverImageResource'
    { _scirHeight :: !(Maybe (Textual Int32))
    , _scirResourceId :: !(Maybe Text)
    , _scirContentHash :: !(Maybe Text)
    , _scirWidth :: !(Maybe (Textual Int32))
    , _scirMimeType :: !(Maybe Text)
    , _scirDownloadURL :: !(Maybe Text)
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'SnapshotCoverImageResource' with the minimum fields required to make a request.
--
...
snapshotCoverImageResource =
  SnapshotCoverImageResource'
    { _scirHeight = Nothing
    , _scirResourceId = Nothing
    , _scirContentHash = Nothing
    , _scirWidth = Nothing
    , _scirMimeType = Nothing
    , _scirDownloadURL = Nothing
    }


-- | The height of the image in pixels.
scirHeight :: Lens' SnapshotCoverImageResource (Maybe Int32)
scirHeight
  = lens _scirHeight (\ s a -> s{_scirHeight = a}) .
      mapping _Coerce
...
-- type it was originally returned for. For example, it\'s not possible to
-- use SnapshotDataResource\'s resource ID as the resource_id of a
-- SnapshotCoverImageResource, even if the blob is a valid image file.
scirResourceId :: Lens' SnapshotCoverImageResource (Maybe Text)
scirResourceId
  = lens _scirResourceId
      (\ s a -> s{_scirResourceId = a})
...
-- resource will not change if the binary data hasn\'t changed. Except in
-- very rare circumstances, the content_hash for matching binary data will
-- be the same within a given player and application.
scirContentHash :: Lens' SnapshotCoverImageResource (Maybe Text)
scirContentHash
  = lens _scirContentHash
      (\ s a -> s{_scirContentHash = a})

-- | The width of the image in pixels.
scirWidth :: Lens' SnapshotCoverImageResource (Maybe Int32)
scirWidth
  = lens _scirWidth (\ s a -> s{_scirWidth = a}) .
      mapping _Coerce

-- | Output only. The MIME type of the image.
scirMimeType :: Lens' SnapshotCoverImageResource (Maybe Text)
scirMimeType
  = lens _scirMimeType (\ s a -> s{_scirMimeType = a})

-- | Output only. A URL the client can use to download the image. May vary
-- across requests, and only guaranteed to be valid for a short time after
-- it is returned.
scirDownloadURL :: Lens' SnapshotCoverImageResource (Maybe Text)
scirDownloadURL
  = lens _scirDownloadURL
      (\ s a -> s{_scirDownloadURL = a})

instance FromJSON SnapshotCoverImageResource where
        parseJSON
          = withObject "SnapshotCoverImageResource"
              (\ o ->
                 SnapshotCoverImageResource' <$>
                   (o .:? "height") <*> (o .:? "resourceId") <*>
                     (o .:? "contentHash")
                     <*> (o .:? "width")
                     <*> (o .:? "mimeType")
                     <*> (o .:? "downloadUrl"))

instance ToJSON SnapshotCoverImageResource where
        toJSON SnapshotCoverImageResource'{..}
          = object
              (catMaybes
                 [("height" .=) <$> _scirHeight,
                  ("resourceId" .=) <$> _scirResourceId,
                  ("contentHash" .=) <$> _scirContentHash,
                  ("width" .=) <$> _scirWidth,
                  ("mimeType" .=) <$> _scirMimeType,
                  ("downloadUrl" .=) <$> _scirDownloadURL])
...
-- developer-provided snapshot_name. The set of attributes and binary data
-- for a specific state is called a revision. Each revision is itself
-- immutable, and referred to by a snapshot revision id. At any time, a
-- snapshot has a \"head\" revision, and updates are made against that
-- revision. If a snapshot update is received that isn\'t against the
-- current head revision, then instead of changing the head revision it
-- will result in a conflicting revision that must be specifically
-- resolved.
--
-- /See:/ 'snapshotExtended' smart constructor.
data SnapshotExtended =
  SnapshotExtended'
    { _seHasConflictingRevisions :: !(Maybe Bool)
    , _seConflictingRevisions :: !(Maybe [SnapshotRevision])
    , _seSnapshotName :: !(Maybe Text)
    , _seHeadRevision :: !(Maybe SnapshotRevision)
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'SnapshotExtended' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'seHasConflictingRevisions'
--
-- * 'seConflictingRevisions'
--
-- * 'seSnapshotName'
--
-- * 'seHeadRevision'
...
snapshotExtended =
  SnapshotExtended'
    { _seHasConflictingRevisions = Nothing
    , _seConflictingRevisions = Nothing
    , _seSnapshotName = Nothing
    , _seHeadRevision = Nothing
    }


-- | An indicator whether the snapshot has any conflicting revisions or not.
-- Always set.
seHasConflictingRevisions :: Lens' SnapshotExtended (Maybe Bool)
seHasConflictingRevisions
  = lens _seHasConflictingRevisions
      (\ s a -> s{_seHasConflictingRevisions = a})

-- | A list of conflicting revisions. Only set if explicitly requested (e.g.
-- using a field mask or a request flag), or if the RPC guarantees that
-- this field is set. The conflicting revisions are sorted chronologically
-- by their server creation time (oldest first). If there are too many
-- conflicting revisions to return all of them in a single request this
-- will only contain the first batch. In such case, the presented
-- conflicting revisions must be resolved first in order to fetch the next
-- batch.
seConflictingRevisions :: Lens' SnapshotExtended [SnapshotRevision]
seConflictingRevisions
  = lens _seConflictingRevisions
      (\ s a -> s{_seConflictingRevisions = a})
      . _Default
      . _Coerce

-- | An identifier of the snapshot, developer-specified. It must match the
-- pattern [0-9a-zA-Z-._~]{1,100}.
seSnapshotName :: Lens' SnapshotExtended (Maybe Text)
seSnapshotName
  = lens _seSnapshotName
      (\ s a -> s{_seSnapshotName = a})

-- | The current head revision (the canonical revision as understood by the
-- server).
seHeadRevision :: Lens' SnapshotExtended (Maybe SnapshotRevision)
seHeadRevision
  = lens _seHeadRevision
      (\ s a -> s{_seHeadRevision = a})

instance FromJSON SnapshotExtended where
        parseJSON
          = withObject "SnapshotExtended"
              (\ o ->
                 SnapshotExtended' <$>
                   (o .:? "hasConflictingRevisions") <*>
                     (o .:? "conflictingRevisions" .!= mempty)
                     <*> (o .:? "snapshotName")
                     <*> (o .:? "headRevision"))

instance ToJSON SnapshotExtended where
        toJSON SnapshotExtended'{..}
          = object
              (catMaybes
                 [("hasConflictingRevisions" .=) <$>
                    _seHasConflictingRevisions,
                  ("conflictingRevisions" .=) <$>
                    _seConflictingRevisions,
                  ("snapshotName" .=) <$> _seSnapshotName,
                  ("headRevision" .=) <$> _seHeadRevision])
...
--
-- /See:/ 'snapshotDataResource' smart constructor.
data SnapshotDataResource =
  SnapshotDataResource'
    { _sdrResourceId :: !(Maybe Text)
    , _sdrSize :: !(Maybe (Textual Int64))
    , _sdrContentHash :: !(Maybe Text)
    , _sdrDownloadURL :: !(Maybe Text)
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'SnapshotDataResource' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'sdrResourceId'
...
snapshotDataResource =
  SnapshotDataResource'
    { _sdrResourceId = Nothing
    , _sdrSize = Nothing
    , _sdrContentHash = Nothing
    , _sdrDownloadURL = Nothing
    }
...
-- The resource ID can only be used within the application, user and
-- resource type it was originally returned for. For example, it\'s not
-- possible to use SnapshotDataResource\'s resource ID as the resource_id
-- of a SnapshotCoverImageResource, even if the blob is a valid image file.
sdrResourceId :: Lens' SnapshotDataResource (Maybe Text)
sdrResourceId
  = lens _sdrResourceId
      (\ s a -> s{_sdrResourceId = a})

-- | Output only. Size of the saved game blob in bytes.
sdrSize :: Lens' SnapshotDataResource (Maybe Int64)
sdrSize
  = lens _sdrSize (\ s a -> s{_sdrSize = a}) .
      mapping _Coerce
...
-- very rare circumstances, the content_hash for matching binary data will
-- be the same within a given player and application.
sdrContentHash :: Lens' SnapshotDataResource (Maybe Text)
sdrContentHash
  = lens _sdrContentHash
      (\ s a -> s{_sdrContentHash = a})

-- | Output only. A URL that the client can use to download the blob. May
-- vary across requests, and only guaranteed to be valid for a short time
-- after it is returned.
sdrDownloadURL :: Lens' SnapshotDataResource (Maybe Text)
sdrDownloadURL
  = lens _sdrDownloadURL
      (\ s a -> s{_sdrDownloadURL = a})

instance FromJSON SnapshotDataResource where
        parseJSON
          = withObject "SnapshotDataResource"
              (\ o ->
                 SnapshotDataResource' <$>
                   (o .:? "resourceId") <*> (o .:? "size") <*>
                     (o .:? "contentHash")
...
instance ToJSON SnapshotDataResource where
        toJSON SnapshotDataResource'{..}
          = object
              (catMaybes
                 [("resourceId" .=) <$> _sdrResourceId,
                  ("size" .=) <$> _sdrSize,
                  ("contentHash" .=) <$> _sdrContentHash,
                  ("downloadUrl" .=) <$> _sdrDownloadURL])
...
--
-- /See:/ 'resolveSnapshotHeadRequest' smart constructor.
data ResolveSnapshotHeadRequest =
  ResolveSnapshotHeadRequest'
    { _rshrMaxConflictsPerSnapshot :: !(Maybe (Textual Int32))
    , _rshrResolutionPolicy :: !(Maybe ResolveSnapshotHeadRequestResolutionPolicy)
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'ResolveSnapshotHeadRequest' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rshrMaxConflictsPerSnapshot'
--
-- * 'rshrResolutionPolicy'
resolveSnapshotHeadRequest
    :: ResolveSnapshotHeadRequest
resolveSnapshotHeadRequest =
  ResolveSnapshotHeadRequest'
    {_rshrMaxConflictsPerSnapshot = Nothing, _rshrResolutionPolicy = Nothing}


-- | The maximum number of SnapshotRevision resources for
-- \`conflictingRevisions\` to return per SnapshotExtended resource in the
-- response. For any response, the actual number of resources returned may
-- be less than specified by \`maxConflictsPerSnapshot\`. The value
-- provided should be greater or equal to 0. If no value is provided, the
-- server will use a sensible default.
rshrMaxConflictsPerSnapshot :: Lens' ResolveSnapshotHeadRequest (Maybe Int32)
rshrMaxConflictsPerSnapshot
  = lens _rshrMaxConflictsPerSnapshot
      (\ s a -> s{_rshrMaxConflictsPerSnapshot = a})
      . mapping _Coerce
...
rshrResolutionPolicy :: Lens' ResolveSnapshotHeadRequest (Maybe ResolveSnapshotHeadRequestResolutionPolicy)
rshrResolutionPolicy
  = lens _rshrResolutionPolicy
      (\ s a -> s{_rshrResolutionPolicy = a})

instance FromJSON ResolveSnapshotHeadRequest where
        parseJSON
          = withObject "ResolveSnapshotHeadRequest"
              (\ o ->
                 ResolveSnapshotHeadRequest' <$>
                   (o .:? "maxConflictsPerSnapshot") <*>
                     (o .:? "resolutionPolicy"))

instance ToJSON ResolveSnapshotHeadRequest where
        toJSON ResolveSnapshotHeadRequest'{..}
          = object
              (catMaybes
                 [("maxConflictsPerSnapshot" .=) <$>
                    _rshrMaxConflictsPerSnapshot,
                  ("resolutionPolicy" .=) <$> _rshrResolutionPolicy])
...
--
-- /See:/ 'snapshotRevision' smart constructor.
data SnapshotRevision =
  SnapshotRevision'
    { _srBlob :: !(Maybe SnapshotDataResource)
    , _srCoverImage :: !(Maybe SnapshotCoverImageResource)
    , _srMetadata :: !(Maybe SnapshotMetadata)
    , _srId :: !(Maybe Text)
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'SnapshotRevision' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'srBlob'
...
srBlob :: Lens' SnapshotRevision (Maybe SnapshotDataResource)
srBlob = lens _srBlob (\ s a -> s{_srBlob = a})

-- | Reference to the cover image for this revision.
srCoverImage :: Lens' SnapshotRevision (Maybe SnapshotCoverImageResource)
srCoverImage
  = lens _srCoverImage (\ s a -> s{_srCoverImage = a})

-- | Metadata for this snapshot revision.
srMetadata :: Lens' SnapshotRevision (Maybe SnapshotMetadata)
srMetadata
  = lens _srMetadata (\ s a -> s{_srMetadata = a})

-- | Output only. A server generated identifier of the snapshot revision.
srId :: Lens' SnapshotRevision (Maybe Text)
srId = lens _srId (\ s a -> s{_srId = a})

instance FromJSON SnapshotRevision where
        parseJSON
          = withObject "SnapshotRevision"
              (\ o ->
                 SnapshotRevision' <$>
                   (o .:? "blob") <*> (o .:? "coverImage") <*>
                     (o .:? "metadata")
...
instance ToJSON SnapshotRevision where
        toJSON SnapshotRevision'{..}
          = object
              (catMaybes
                 [("blob" .=) <$> _srBlob,
                  ("coverImage" .=) <$> _srCoverImage,
...
--
-- /See:/ 'resolveSnapshotHeadResponse' smart constructor.
newtype ResolveSnapshotHeadResponse =
  ResolveSnapshotHeadResponse'
    { _rshrSnapshot :: Maybe SnapshotExtended
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'ResolveSnapshotHeadResponse' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rshrSnapshot'
resolveSnapshotHeadResponse
    :: ResolveSnapshotHeadResponse
resolveSnapshotHeadResponse =
  ResolveSnapshotHeadResponse' {_rshrSnapshot = Nothing}


-- | The state of the snapshot.
rshrSnapshot :: Lens' ResolveSnapshotHeadResponse (Maybe SnapshotExtended)
rshrSnapshot
  = lens _rshrSnapshot (\ s a -> s{_rshrSnapshot = a})

instance FromJSON ResolveSnapshotHeadResponse where
        parseJSON
          = withObject "ResolveSnapshotHeadResponse"
              (\ o ->
                 ResolveSnapshotHeadResponse' <$> (o .:? "snapshot"))

instance ToJSON ResolveSnapshotHeadResponse where
        toJSON ResolveSnapshotHeadResponse'{..}
          = object
              (catMaybes [("snapshot" .=) <$> _rshrSnapshot])
...
--
-- /See:/ 'snapshotMetadata' smart constructor.
data SnapshotMetadata =
  SnapshotMetadata'
    { _smLastModifyTime :: !(Maybe DateTime')
    , _smProgressValue :: !(Maybe (Textual Int64))
    , _smGameplayDuration :: !(Maybe GDuration)
    , _smDeviceName :: !(Maybe Text)
    , _smDescription :: !(Maybe Text)
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'SnapshotMetadata' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'smLastModifyTime'
...
snapshotMetadata =
  SnapshotMetadata'
    { _smLastModifyTime = Nothing
    , _smProgressValue = Nothing
    , _smGameplayDuration = Nothing
    , _smDeviceName = Nothing
    , _smDescription = Nothing
    }


-- | The timestamp of the last modification to this snapshot as provided by
-- the client. Values with sub-millisecond precision can be rounded or
-- trimmed to the closest millisecond.
smLastModifyTime :: Lens' SnapshotMetadata (Maybe UTCTime)
smLastModifyTime
  = lens _smLastModifyTime
      (\ s a -> s{_smLastModifyTime = a})
      . mapping _DateTime

-- | The progress value (64-bit integer set by developer) associated with
-- this snapshot.
smProgressValue :: Lens' SnapshotMetadata (Maybe Int64)
smProgressValue
  = lens _smProgressValue
      (\ s a -> s{_smProgressValue = a})
      . mapping _Coerce
...
smGameplayDuration :: Lens' SnapshotMetadata (Maybe Scientific)
smGameplayDuration
  = lens _smGameplayDuration
      (\ s a -> s{_smGameplayDuration = a})
      . mapping _GDuration

-- | The device that created the current revision.
smDeviceName :: Lens' SnapshotMetadata (Maybe Text)
smDeviceName
  = lens _smDeviceName (\ s a -> s{_smDeviceName = a})

-- | The description of this snapshot.
smDescription :: Lens' SnapshotMetadata (Maybe Text)
smDescription
  = lens _smDescription
...
instance FromJSON SnapshotMetadata where
        parseJSON
          = withObject "SnapshotMetadata"
              (\ o ->
                 SnapshotMetadata' <$>
                   (o .:? "lastModifyTime") <*> (o .:? "progressValue")
                     <*> (o .:? "gameplayDuration")
                     <*> (o .:? "deviceName")
                     <*> (o .:? "description"))
...
instance ToJSON SnapshotMetadata where
        toJSON SnapshotMetadata'{..}
          = object
              (catMaybes
                 [("lastModifyTime" .=) <$> _smLastModifyTime,
                  ("progressValue" .=) <$> _smProgressValue,
                  ("gameplayDuration" .=) <$> _smGameplayDuration,
                  ("deviceName" .=) <$> _smDeviceName,
