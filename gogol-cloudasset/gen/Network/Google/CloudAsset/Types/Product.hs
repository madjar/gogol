...
-- \`accessPolicies\/MY_POLICY\/accessLevels\/MY_LEVEL\`. If \`*\` is
...
-- that if the destination of the request is protected by a
...
-- destination of the request is protected by a ServicePerimeter, then that
...
-- that match this to stanza. A request matches if it contains a resource
-- in this list. If \`*\` is specified for resources, then this EgressTo
...
-- | A list of ApiOperations that this egress rule applies to. A request
-- matches if it contains an operation\/service in this list.
...
-- sources defined in the corresponding IngressFrom. A request matches if
-- it contains a resource in this list. If \`*\` is specified for
...
-- | A list of ApiOperations the sources specified in corresponding
